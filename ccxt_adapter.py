...
class CcxtDataProvider:
    ...
    def __init__(self, config):
        self.provider = get_market_data_provider(config.get("provider") or config.get("exchange"))
        self.exchange = ccxt.binance() if config.get("exchange") == "binance" else ccxt.exchange(config.get("exchange", "binance"))
